...
import { createPartition } from "../api.js";
...
export default function AddPartitionModal({ onClose }: AddPartitionModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await createPartition(name.trim());
    ...
    <ModalOverlay onClose={onClose}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        <div>
          ...
          <h2 className="modal-title">Add Partition</h2>
        </div>
      ...
      <form onSubmit={handleSubmit}>
        <p style={{ fontSize: 13, color: "var(--text-muted)", lineHeight: 1.55, margin: "0 0 18px 0" }}>
          Partitions are logical groupings with scoped variables and constraints. You can configure variables after creation.
        </p>
        ...
        >
          {saving ? "Creating…" : "Create Partition"}
        </button>
      </form>
    </ModalOverlay>
  );
