...
**Planning:** The LLM reasons freely — analyzing artifacts, probing infrastructure, detecting conflicts, producing a deployment plan. This phase is read-only. No side effects. It can think as hard as it needs to.
...
Building is the easy part. For someone like me, anyway. Something about being able to steer and direct a project — shaping the architecture, making the product decisions — without having to constantly worry about the code working... it's the most creatively free I've felt in my career. Having a deep technical background means I know when to push back on what the AI suggests. But it doesn't help with the other side of this.
